...
at the end of the file, which saves all datasets and variables that currently existed in the data set.  Then I reload the data using the `load` command in the following code chunk.
...
Results with standard deviations of error calculations
==============
```{r}
cartTune$results %>% head()
```
...
library(rpart)
library(rpart.plot)
rpart.plot(cartTune$finalModel)
```
...
```{r}
ggplot(cartTune$results, aes(x=cp, y=RMSE)) + 
  geom_pointrange(aes(ymin = RMSE-RMSESD, ymax=RMSE+RMSESD)) + geom_line() + 
  scale_x_log10() + ylab("RMSE (Crossvalidation)") + xlab("Complexity parameter (log10(cp))")
...
-  Different methods apply different ways of creating the models
-  Random forests - Models differ by allowable predictors
-  Bagging - Random generations of datasets through Bootstrap selection
-  Boosting - Each iteration oversamples where prior models performed poorly
...
- Because each tree has different predictors, they should make different predcitions.
...
![](../resources/Random_forest_diagram_complete.png)
...
- mtry:  Number of chosen variables in each tree
- length:  How many values of mtry to try
```{r} 
...
- Try ggplot because built in `plot` command does not show SD

```{r}
ggplot(rfTune$results, aes(x=mtry, y=RMSE)) + 
  geom_pointrange(aes(ymin = RMSE-RMSESD, ymax=RMSE+RMSESD)) + geom_line() + 
   ylab("RMSE (Crossvalidation)") + xlab("Number of randomly selected variables (mtry)")
...
-  Create different model through use of bootstrapping to construct different data sets.
...
Note: set multicore (`do`) for parallelization
```
{r}
library(doMC)
registerDoMC(2)
...
2.  Compute average response $\bar{y}$ as the average predicted value for each sample (i.e. no model)
3.  For $k$ in 1 to $K$
...
-  Methods include *Adaboost* and *Gradient Boosting Machine* (GBM)
...
```{r}
ggplot(gbmTune$results, aes(x=n.trees, y=RMSE, color=as.factor(interaction.depth))) + geom_pointrange(aes(ymin=RMSE-RMSESD, ymax=RMSE+RMSESD)) + geom_line() + facet_grid(. ~ as.ordered(shrinkage))
```
...
-  Ensemble models work by having a large number of models that differ, so they vote on the eventual outcome
