---
title: 'Lecture 10: Linear Classification Methods'
...
load("classificationMethods.Rdata")
...
-  Use `predict` function to use new data on a created model
...
====
```{r}
dayProfile <- predict(daySquaredModel,
                      Day = 0:365,
                      fun=function(x) -x)
histogram(dayProfile, ylab="Log Odds")
```
...
- Another way of thinking is to maximize between-group variance compared to within-group variance.
...
ldaModel <- train(training[, reducedSet],
               y=training$Class,
               method = "lda",
               metric ="ROC",
               trControl = ctrl)
...
- Using linear discriminant analysis with scaling
```{r}
set.seed(476)
...
===================
```{r}
reducedldaRoc <- roc(response = ldaFit1$pred$obs,
               predictor = ldaFit1$pred$successful,
               levels = rev(levels(ldaFit1$pred$obs)))
auc(reducedldaRoc)
plot(reducedldaRoc, legacy.axes = TRUE)
```

```{r}
save.image("classificationMethods.Rdata")
```
