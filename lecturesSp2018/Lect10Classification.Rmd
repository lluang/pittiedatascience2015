---
title: 'Lecture 10: Classification models Evaluation'
...
2. OneR - Rule based on one predictor
...
3. This often matches expert intuition.  Consider a single factor to make a prediction.
...
-  For a given value of model probability output, what was the actual probability of the event occurring?
-  Which is better?

```{r}
ggplot(calData1$data) + geom_point(mapping=aes(midpoint, Percent, color=calibModelVar)) +
  geom_line(mapping=aes(midpoint, Percent, color=calibModelVar, show.legend=TRUE)) +
  geom_abline() +
  scale_x_discrete() +
  ggtitle("Calibration plot comparing QDA and random forest classifier probability predictions") + theme_bw()
```
...
Recalibrate as if predicting probabilities using Naive Bayes or QDA with sigmoidal (binomial) functions
============
```{r}
...
=====================
```{r}
ggplot(calData2$data) + geom_point(mapping=aes(midpoint, Percent, color=calibModelVar)) +
  geom_line(mapping=aes(midpoint, Percent, color=calibModelVar)) +
  geom_abline() + xlab("Predicted probability") + ylab("Observed event probability")
  ggtitle("Calibration plot comparing QDA, and Bayesian and Sigmoidal Calibration") + theme_bw()
...
==================
-  Note that the *y* variable is a Factor
```{r}
...
```{r}
ggplot(creditResults) + geom_histogram(aes(prob)) + facet_grid(. ~ Label) +
          xlab("Probability of Bad Credit") + theme_bw()
```
...
```{r}
ggplot(creditCalib$data) + geom_point(mapping=aes(midpoint, Percent, color=calibModelVar)) +
  geom_line(mapping=aes(midpoint, Percent, color=calibModelVar)) +
  geom_abline() +
  xlab("Predicted probability") + ylab("Observed event percentage") +
  ggtitle("Calibration plot for Bad Credit") + theme_bw()
```
