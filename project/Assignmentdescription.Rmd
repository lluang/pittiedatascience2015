---
title: IE 2064 Data Science Project Description
date: Spring 2019
output:
  pdf_document: default
  html_document: default
---
Course project for IE 2064 Data Science
====================================

The goal of this course is to gain practical skills, knowledge, and understanding in the analysis of data to make data driven decisions.  The grading in this course is built around three presentations on a single topic, where the presentations demonstrate increasing understanding of the data, the problem domain, and the application of methods for processing, analyzing, and presenting the data, question, and results.  The project may be done individually or a group of 2.

For a projecct dataset and problem, you may look at Kaggle (http://www.kaggle.com) for datasets.  Just as important, the Kaggle competition datasets are sponsored by a company that has an actual problem, so you may write the CONVO portion of the assignment based on that company. Note: if you use a Kaggle dataset, it must be a true competition, as these have an actual problem and setting. So you may not choose a dataset with any of following labels: *getting started*, *playground*, or *knowledge*.  You may choose *research*, but look carefully before you do this to make sure that you will be able to write a useable CONVO statement, as this tends to determine a successful project.

Each assignment for the project consists of a short paper and a presentation. The paper should be written in R Markdown format (delivered as a pdf) and the presentation should be written in R Presentation format (delivered as HTML or PDF).  The format should follow the outline given in the case study report format document.

For parts 1 and 2, the paper should be around 4-5 pages (not counting figures), the presentation should be 5-8 minutes.

For the final project, the paper should be around 8-10 pages (not counting figures), the presentation should be 10-12 minutes.

The requirements for the project are below, so the data set you choose should be rich enough so that these tasks can be done.  However, if you have a data analysis that needs to be done for some other purpose (work data, thesis, research project) see me and we can adapt the project so that it contributes to your other work.
...
1.  Exploratory data analysis (February 14)
2.  Preliminary data analysis (March 14)
3.  Final report and presentation (April 25)
...
5.  Identify an outcome and three independent variables based on your CoNVO statement and the pairs plot.  Use at least three plot types for this part (geoms) (Note: the easy way to do this is to take from the five named plot types, but you may find more complex plots are more useful.
6.  Design and create a single plot that shows the effects of all three independent variables in question 6. Explain and interpret the plot.  Be sure to reference the CoNVO statement in question 4.
7.  Working with the data - What were difficulties faced while preparing the data for analysis?
...
4.  Apply methods using set of methods following the same rules as the applicable homework assignment (e.g. is this a regression problem or a classification problem)
6.  Plan of work.  Identify what class of methods you will be comparing in the final project.  Note: 
...
5.  Analysis - Methodology. How it relates predictors to outcomes. Compare several predictive analytics methods from different families and discuss how they preform?  Why do some methods work better than others?
6.  Conclusions - Note: It is not necessary for there to be a 'successful' application of data mining methods that 'solves' the problem. But you should explain how the methodology you applied failed. (i.e. it may imply that the relationship hypothesized does not exist, although if you say this, you should probably have tried either additional methodologies or an additional hypothesis before reaching this conclusion)
