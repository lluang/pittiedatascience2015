...
```{r}
attend <- read_table2("../data/afl.txt")
head(attend)
...
ggplot(attend, aes(Members, MCG)) + 
  geom_point()+ geom_line(data=predict1) + #Note that both geom share same aesthetics
  theme_bw() +
  xlab("Combined membership of teams playing") + 
  ylab("MCG Match Day Attendance" )
...
- Note improvement in standard error and adjusted R-squared
...
```{r}
ggplot(attend, aes(Members, MCG)) + 
  geom_point(aes(color=away.inter)) + 
  geom_line(data=predict2)+ 
  xlab("Combined membership of teams playing") + 
  ylab("MCG Match Day Attendance" ) + theme_bw()
```
...
- *caret* (Classification And REgression Training) is a library for applying machine learning
    - Wraper for a large number of machine learning packages
    - Provides a common API for applying the methods to problems
    - Provides for model evaluation framework
        - Cross validation


# Working with `caret`

-  The `caret` package rationalizes all of the machine learning methods that exist
    -  Packages are written by subject matter experts
    -  `caret` provides a consistant interface between the analyst and all of the other packages.
-  Workflow
    - `set.seed()` - Repeatability of results of random sampling
    - `createFolds()` - Sets parameters for generating partitions for validation.  Default is 10 partitions.
    - `tuneGrid()` - Specifies tuning paramters (complexity parameters)
    - `trainControl()` - Generates partitions. Specifies resampling methods and other parameters (usually specified in `createFolds`)
    - `train` - Uses training data to parameterize a model using the validation method specified.
...
- Hypothesis based statistics used a hypothesis test to evaluate models.
- In predictive models, we often do not have a hypothesis to test. 
    - Goal is to make a good prediction, not necessarily a statement about truth.
- Use a measure assessing ability to predict the value of an observation not used in creating the model
- Standard method, *k-fold cross validation*
- Divide up training data into segments (folds).
- For each fold, use the remaining data to develop a model to predict outcome in that fold.
- Now have *k* estimates of measures of error, so can calculate mean and standard deviation.


![Crossvalidation ](../resources/kfoldcrossvalidation.jpg)
...
Comparison of linear regression and recursive partition
=============

- Look at measure of error (e.g. RMSE)
- Consider complexity of model (will discuss later)
- Compare measure of error to best + standard deviation
...
-  If there are a large number of predictors, another alternative to removing high correlation variables is to use Principle Component Analysis on the predictors.
-  Resulting principle components are uncorrelated (by definition)

# Partial Least Squares

- Used when there are correlated predictors.
-  Method finds underlying relationships among the predictors that are correlated with the response.
  -  Relationship between predictors and response is the *direction* **W**
...
-  Uses another penalty function, similar to ridge regression
