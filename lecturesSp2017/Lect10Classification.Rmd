...
- Prediction accuracy
  -  ROC
  -  Lift charts
- Goal is to find the model that offers the best performance
  - The problem may be inherently difficult to predict.
...
set.seed(100)
inTrain <- createDataPartition(GermanCredit$credit_risk, p = .8)[[1]]
GermanCreditTrain <- GermanCredit[ inTrain, ]
GermanCreditTest  <- GermanCredit[-inTrain, ]

set.seed(1056)
logisticReg <- train(credit_risk ~ .,
...
===
```{r}
creditResults <- data.frame(obs = GermanCreditTest$credit_risk)
...
-  *Specificity* - samples that were predicted to be false and were falce compared to the total number of samples that were false
...
==============================
- ROC curve takes a model and allows its paramaters to range through a set of values.
...
1. Predict a set of test samples (not used in bulding the model, but outcomes are known)
