...
- Candle[stick] chart has a the bar for open/close, with whiskers for the high/low. 
...
nnetGrid <- expand.grid(size = seq(from = 1, to = 5, by = 2),
                        decay = seq(from = 0.1, to = 0.5, by = 0.2))
nncaret <- train(x = pricemodel[,-1], 
            y = pricemodel[,"Tind"],
            preProcess = 'scale',
            method='nnet',
            linout=T,trace=F,
...
```
{r}
preds <- predict(nncaret, Tdata.train[1001:2000,])
...
```{r nncaret}
nncaret <- train(x = Tdata.train[1:1000,-1], 
            y = Tdata.train[1:1000,1],
            preProcess = 'scale',
            method='nnet',
            linout=T,trace=F,
            trControl = ctrl)
...
```{r nnsignals}
nnsignals <- train(x = Tdata.train[1:1000,-1], 
            y = signals[1:1000],
            preProcess = 'scale',
            method='nnet',
            linout=T,trace=F,
...
```{r}
preds <- predict(nncaret, Tdata.train[1001:2000,])
sigs.nnclass <- trading.signals(preds, 0.1, -0.1)
true.sigs <- trading.signals(Tdata.train[1001:2000,'T.ind.GSPC'],0.1,-0.1)
confusionMatrix(sigs.nnclass,true.sigs)
```
...
-First, use version in `e1071` package.
```{r svme1071}
library(e1071)
svme1701 <- train(x = Tdata.train[1:1000,-1], 
            y = signals[1:1000],
            method='svmLinear2',
            trControl = ctrl)
```
...
```{r}
s.preds <- predict(svme1701,Tdata.train[1001:2000,])
true.sigs <- trading.signals(Tdata.train[1001:2000,'T.ind.GSPC'],0.1,-0.1)
confusionMatrix(s.preds,true.sigs)
```
...
```{r svmkernlab}
svmkernlab <- train(x = Tdata.train[1:1000,-1], 
            y = signals[1:1000],
            method='svmLinear',
            trControl = ctrl)
```
...
```{r}
kernlab.preds <- predict(svme1701,Tdata.train[1001:2000,])
#true.sigs <- trading.signals(Tdata.train[1001:2000,'T.ind.GSPC'],0.1,-0.1)
confusionMatrix(kernlab.preds,true.sigs)
```
...
```{r}
mars <- train(x = Tdata.train[1:1000,-1], 
            y = signals[1:1000],
            method='earth',
            trControl = ctrl)
e.preds <- predict(mars,Tdata.train[1001:2000,])
sigs.e <- trading.signals(e.preds,0.1,-0.1)
true.sigs <- trading.signals(Tdata.train[1001:2000,'T.ind.GSPC'],0.1,-0.1)
confusionMatrix(sigs.e,true.sigs)
```
...
kernlab.preds <- predict(svme1701,Tdata.train[1001:2000,])
#true.sigs <- trading.signals(Tdata.train[1001:2000,'T.ind.GSPC'],0.1,-0.1)
confusionMatrix(krenlab.preds, true.sigs)
```
...
```{r}
save.image(file="nonlinearRegression.Rdata")
```